...
def gen_labels_forone(df, sequence_length, label):
    data_matrix = df[label].values
    num_elements = data_matrix.shape[0]
...
def gen_label_wrapper_forone(df, sequence_length, label, unit_nr):
    # if unit_nrs.size <= 0:
    #     unit_nrs = df['unit_nr'].unique()
        
    label_gen = gen_labels_forone(df[df['unit_nr']==(unit_nr)], sequence_length, label)
...
def get_oneUnitData(dataset, sensors, sequence_length, alpha, threshold, step=1, unit_nr=1):
    ...
    y_test_oneunit = gen_label_wrapper_forone(X_test_pre, sequence_length, ['RUL'], unit_nr)
    x_test = np.array(test_gen).astype(np.float32).reshape(-1,30,len(sensors))


    return x_test, y_test_oneunit
# ---------------------------------------------------------------------------------------------------
class EarlyStopping:
